...
module.exports = class Galileo extends FlightSearch {
  ...
  raisePrice(priceInfo, profitInfo, type) {
    ...
    if (raiseType === 'percent' || !raiseType) {
      Object.keys(priceInfo).forEach(key => {
        priceInfo[key].salePrice = Number((priceInfo[key].salePrice * (percent / 100) + trim * this.params.cnyRate).toFixed(0));
        priceInfo[key].tax = Number((priceInfo[key].tax * (percent / 100)).toFixed(0));
      });
    } else {
      Object.keys(priceInfo).forEach(key => {
        priceInfo[key].salePrice = Number((fixedPrice * this.params.cnyRate).toFixed(0));
        priceInfo[key].tax = Number((fixedTax * this.params.cnyRate).toFixed(0));
      });
    }
    return priceInfo;
  }
  ...
  async resolveRenvenuInfo() {
    if (!this.params.revenueId) {
      return;
    }
    const revenueInfo = await this.app.ctx.model.Revenue.findOne({
      revenueId: this.params.revenueId,
    });
    this.storedData.policyDetailInfo = this.raisePrice(this.storedData.policyDetailInfo, revenueInfo, 'revenueType');
  }

  async resolveProfitInfo() {
    if (!this.params.profitId) {
      return;
    }
    const profitInfo = await this.app.ctx.model.Profit.findById(this.params.profitId);
    this.storedData.policyDetailInfo = this.raisePrice(this.storedData.policyDetailInfo, profitInfo, 'profitType');
    this.storedData.policyDetailInfo.profitInfo = profitInfo;
  }

  async resovePenaltyAndBaggageInfo() {
    // 获取baggage
    const baggageInfo = this.params.baggageId ? [ await await this.app.ctx.model.Baggage.findById(this.params.baggageId) ] : [];
    // 获取退改签

    const defaultPenalty = this.params.penaltyId ? [ await this.app.ctx.model.Penalty.findById(this.params.penaltyId) ] : [];
    const baggageInfoList = this.app.service.baggage.generateBaggageInfoNew(baggageInfo, this.storedData, this.params.group, this.params.IPCC, this.params.departTime);
    const penaltyInfoList = this.app.service.penalty.generatePenaltyInfo(defaultPenalty, this.storedData, this.params.departTime);
